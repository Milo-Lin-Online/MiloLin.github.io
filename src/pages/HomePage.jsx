...
import { content, toParagraphs } from '../content/loadContent.js';
...
export default function HomePage({ portfolio, onNavigate }) {
  ...
  const homeReel = {
    file: content.home.reel,
    link: content.home.reelLink,
    fallback: profile.reelUrl,
  };
  ...
  return (
    <main id="main" className="page">
      <div className="wrap">
        ...
        {/* Reel from content/REEL.mp4, content/REEL.txt, or profile.reelUrl. */}
        {hasReel(homeReel) && (
          <section className="section">
            <h2 className="section-label">Demo Reel</h2>
            <ReelPlayer {...homeReel} label="Play demo reel" />
          </section>
        )}
        ...
        <section className="section">
          ...
          <div className="panel about">
            {/* content/about.txt replaces this copy if you create it. */}
            {(content.home.about ? toParagraphs(content.home.about) : profile.about).map(
              (paragraph, i) => (
                <p key={i}>{paragraph}</p>
              )
            )}
