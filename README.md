...
**Current version: v1.7.0** — shown faintly in the bottom-right corner of the page.
...
The nav was the real breakage: three lane links plus two icons couldn't fit a
phone-width bar, so it wrapped to two rows and sat on top of the page content.

- **Below 720px the links move into a drawer** behind a menu button. The bar
  stays a fixed 56px, so `padding-top` on the page always clears it.
- Tapping a link navigates *and* closes the drawer; the page behind it is
  locked from scrolling while it's open.
- **Touch targets** are 44px+ throughout — menu button, drawer rows, modal
  close, connect buttons, chips.
- **Modals go full-screen** on phones with a sticky header, instead of a small
  floating box, and galleries drop to one column.
- `min-height: 100svh` rather than `100vh`, so the layout doesn't jump when
  mobile browser chrome hides.
- `overflow-x: hidden` as a backstop, and verified with no sideways scroll at
  **320, 360, 390, 430, 768, 1024 and 1440px**.
- The fish cursor stays off on touch devices — it checks `pointer: fine`.

## The click splash

`src/services/SplashEffect.js`, mounted by `src/components/SplashLayer.jsx`.

Every click or tap drops a small water splash: two thin expanding rings plus
seven droplets thrown outward and pulled back down by gravity. Strokes only —
no `shadowBlur`, no radial gradients — so it stays consistent with the flat
hairline look rather than reintroducing glow.

How it stays out of the way:

- One `<canvas>` and one `requestAnimationFrame` loop for the whole page, and
  the loop **stops itself** when the last particle dies. Idle page, zero frames.
- `pointer-events: none` on the canvas, so every click passes straight through
  to the button underneath.
- The listener is passive and registered in the capture phase, so a splash still
  appears even where a handler stops propagation — and it never calls
  `preventDefault`, so scrolling and buttons behave normally.
- No React state involved, so clicking never causes a re-render.
- Disables itself under `prefers-reduced-motion`.

To tune it, edit the `splash()` method — ring `maxR` and `speed`, droplet
`count`, and the gravity constant in `_tick()`.


---

## ✏️ Adding work — read `ADDING-WORK.md`

**You no longer edit code to add a project.** Drop folders into `content/` and
the site builds itself around them:

```
content/3D Art/1_Board.fun/
    description.txt
    1_blockout.png
    2_prop study.png
```

Missing file = missing section. No `REEL.mp4` means no reel block at all, rather
than an empty placeholder. Galleries reshape to however many images the folder
holds. A folder with a new section name creates a new page and adds it to the
nav on its own.

Full guide with all the rules: **`ADDING-WORK.md`** (same file lives at
`content/README.md`, next to where you'll be working).


---

## 🚨 The live site is stale — read `DEPLOY.md` first

Your source on `main` is fine. GitHub Pages serves a **build**, not source, and
the only build ever published is the original one on a `gh-pages` branch. Every
commit since has pushed source only, so the live site never changed.

`.github/workflows/deploy.yml` now builds and deploys on every push. To switch it
on you have to flip one setting yourself:

**Settings → Pages → Build and deployment → Source → "GitHub Actions"**

Then delete the two leftover files (`tailwind.config.js`, `eslint.config.js`) and
push. Full walkthrough in **`DEPLOY.md`**.
...
or just look for the teal startup line it logs. If it doesn't say **v1.7.0**,
